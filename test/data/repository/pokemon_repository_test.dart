...
void main() {
  ...
  group('PokemonRepository Tests', () {
    ...
    final tPokemonListResponse = {
      ApiKeys.results: [
        {ApiKeys.name: tPokemonName, ApiKeys.url: 'https://pokeapi.co/api/v2/pokemon/$tPokemonId/'}
      ]
    };
    ...
    group('getAllPokemon', () {
      test('should return list of pokemon when call is successful', () async {
        // arrange
        when(() => mockApiClient.safeGet(any(), query: any(named: 'query')))
            .thenAnswer((_) async => Response(body: tPokemonListResponse, statusCode: 200));
    ...
    group('getPokemonDetail', () {
      test('should return cached detail when available and cache is enabled', () async {
        // arrange
        when(() => mockSettings.useCache).thenReturn(true);
        when(() => mockStorage.read(any())).thenReturn(tPokemonDetailResponse);

        // act
        final result = await repository.getPokemonDetail(tPokemonId, name: tPokemonName);
      ...
      test('should fetch from network and save to cache when not in cache', () async {
        // arrange
        when(() => mockSettings.useCache).thenReturn(true);
        when(() => mockStorage.read(any())).thenReturn(null);
        when(() => mockApiClient.safeGet(any()))
            .thenAnswer((_) async => Response(body: tPokemonDetailResponse, statusCode: 200));
